...
int Liquid::get_flow_direction() {return flow_direction;}
void Liquid::set_flow_direction(int value) {flow_direction = value;}

int Liquid::get_flow_speed() {return flow_speed;}
void Liquid::set_flow_speed(int value) {flow_speed = value;}

int Liquid::get_evaporation() {return evaporation;}
void Liquid::set_evaporation(int value) {evaporation = value;}

void Liquid::update_cell_position(Cell* old_world[][WORLD_HEIGHT], Cell* new_world[][WORLD_HEIGHT], int x, int y) {
    // TODO: Do you really need to call get_updated_this_frame?
    if (get_updated_this_frame() || (old_world[x][y+1]->get_density() > get_density() && old_world[x-1][y]->get_density() > get_density() && old_world[x+1][y]->get_density() > get_density()))  {
        return;
    }
    ...
    // TODO: MAYBE STUFF BELOW IS UNIQUE TO EACH LIQUID FOR EFFETS AND WHAT NOT. MOVEMENT IS THE SAME BUT NOT WAHT LAVA DOES TO WATER VS OIL?
    // Execute move
